"""Read-only checks; no model fitting."""
...
def check():
    missing = [name for name in ("lightgbm", "sklearn", "joblib") if importlib.util.find_spec(name) is None]
    if missing:
    ...
    return {
        ...
        "semantic_ready": True,
        "sample_label_counts": {
            f"{horizon}d": {str(key): int(value) for key, value in sample[f"alternative_{horizon}d"].value_counts().items()}
