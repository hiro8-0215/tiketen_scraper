...
class TimelineTest(unittest.TestCase):
    def test_listing_is_censored_at_its_last_observation(self):
        tickets=self._tickets()
        tickets.loc[tickets.ticket_id.eq('active'),'last_observed_at']=pd.Timestamp('2026-01-02')
        prepared,_=add_end_times(tickets,cutoff=pd.Timestamp('2026-01-10'))
        self.assertEqual(prepared.set_index('ticket_id').loc['active','known_until'],pd.Timestamp('2026-01-02'))
